  ...
  if (algorithm  == "tabu") {
    ...
    if (rep==1){
      ...
      boot = boot.strength(data = expression, R =rep,algorithm = "hc",
                           algorithm.args = list(score = "bic", blacklist =black.list,tabu = 100,start=start,debug = debug))
...
for (i in seq(1)){
  start= empty.graph(nodes=c(regulators,targets),1)
  ...
  write.bif(file='learned_network.bif',bn.est)
}


#num.perturb=ceiling(dim(arcs(bn.learn))[1]*0.05)
#bn.list=vector("list",10)
#for (bn.old in bn.list)
#{
#  bn.new=bn.learn
